...
void CThiefLeg::drawLeg(float tightLength, int type, float thighJointX, float thighJointY, int thighAngle, bool thighDirection)
{
	jointY = tightLength;
	switch (type)
	{
...
void CThiefLeg::setLeftAngle(bool thighDirection, float thighAngle)
{
	...
	case 3:
		...
		//startTick = GetTickCount64();
		leftAngle = -45 - ((currentTick - startTick) / (VELORATE - ADJUST * passCount));
		//cout << startTick << " " << currentTick << endl;
...
void CThiefLeg::setRightAngle(bool thighDirection, float thighAngle)
{
	if (thighDirection && thighAngle < 0) {
		//중간에서 뒤로 접히는 구간
		//최대로 펴져서 이제 접히기 시작하는 부분
...
void CThiefLeg::setPassCount(int passCount)
{
